...
for f in *sorted.bam
        do
                java -jar /data/home/simontj/Tools/picard.jar  MarkDuplicates I=$f O=${f%.*}.dup.bam  M=${f%.*}.marked_dup_metrics.txt
        done
wait
...
for f in *sorted.dup.bam;
        do
                ...
                macs2 callpeak -t "$f" -c “input.bam” -f BAM -g hs -n ${f%.*} -q 0.05
        done
