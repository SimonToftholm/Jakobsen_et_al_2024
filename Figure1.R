...
################# Supp 1h ################# 
brca_tcga<-read.delim("brca_tcga_clinical_data.tsv") #Import everything for TCGA breast cancer
...
ER_neg <- brca_tcga[brca_tcga$ER.Status.By.IHC == "Negative",] # Get ER postive and negative
ER_pos <- brca_tcga[brca_tcga$ER.Status.By.IHC == "Positive",]
...
diff_intergenic_Meso_50kb <- read.delim("Meso_MYC_Enh.Gene.50kb.txt")
diff_intergenic_Meso_50kb <- unique(diff_intergenic_Meso_50kb)
diff_intergenic_Meso_50kb$gene <- gsub(";", "",diff_intergenic_Meso_50kb$ensembl_havana.)

diff_intergenic_Recep_50kb <- read.delim("ER_MYC_Enh.Gene.50kb.txt")
diff_intergenic_Recep_50kb <- unique(diff_intergenic_Recep_50kb)
diff_intergenic_Recep_50kb$gene <- gsub(";", "",diff_intergenic_Recep_50kb$havana.)

ER_neg_exp_Meso <- ER_neg_exp[rownames(ER_neg_exp) %in% diff_intergenic_Meso_50kb$gene,]
ER_pos_exp_Meso <- ER_pos_exp[rownames(ER_pos_exp) %in% diff_intergenic_Meso_50kb$gene,]

ER_neg_exp_Recep <- ER_neg_exp[rownames(ER_neg_exp) %in% diff_intergenic_Recep_50kb$gene,]
ER_pos_exp_Recep <- ER_pos_exp[rownames(ER_pos_exp) %in% diff_intergenic_Recep_50kb$gene,]

sum_neg_meso <- as.data.frame(colMeans(ER_neg_exp_Meso))
sum_pos_meso <- as.data.frame(colMeans(ER_pos_exp_Meso))
sum_neg_recep <- as.data.frame(colMeans(ER_neg_exp_Recep))
sum_pos_recep <- as.data.frame(colMeans(ER_pos_exp_Recep))

sum_neg_meso$type <- rep("sum_neg_meso")
sum_pos_meso$type <- rep("sum_pos_meso")
sum_neg_recep$type <- rep("sum_neg_recep")
sum_pos_recep$type <- rep("sum_pos_recep")

colnames(sum_neg_meso) <- c("FPKM","type")
colnames(sum_pos_meso) <- c("FPKM","type")
colnames(sum_neg_recep) <- c("FPKM","type")
colnames(sum_pos_recep) <- c("FPKM","type")

plot <- rbind(sum_neg_meso, sum_pos_meso, sum_neg_recep, sum_pos_recep)
plot$type <- factor(plot$type, c("sum_neg_meso","sum_pos_meso","sum_neg_recep","sum_pos_recep"))

ggplot(plot, aes(x=type, y=log(FPKM), fill = type)) + geom_violin(outlier.alpha = 1, alpha = 0.8) + geom_boxplot(alpha = 0.4, outlier.alpha = 0) +
  theme(axis.title = element_text(size = 20),
        axis.text = element_text(size = 20, colour = "black"),
        panel.background = element_blank(),
        panel.grid = element_blank(),
        axis.line = element_line(color = "black"),panel.border = element_rect(colour = "black", fill = NA), legend.position = "bottom")+
  xlab(" ") + ylab("Log(FPKM)") + scale_fill_viridis_d(option="E") + coord_cartesian(ylim=c(6.7,8.2))
